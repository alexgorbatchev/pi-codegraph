...
const ansiEscapePattern = new RegExp(`${String.fromCodePoint(27)}\\[[0-9;]*m`, "g");
...
export function annotateFilesResult(text, originalPath) {
    if (!originalPath || !String(text).includes(emptyFilesMarker))
        return text;
...
export async function resolveCodeGraphLaunch(settings, args = []) {
    ...
    try {
        ...
        const binValue = typeof packageJson?.parsed?.bin === "string"
    ...
    return new Promise((resolvePromise, reject) => {
        ...
        child.on("exit", (code) => {
            if (code === 0)
                ...
                finish(new Error(sanitizeDiagnostic(stderr) ||
...
async function resolvePathSafe(inputPath) {
    try {
...
async function readIndexedSourcePath(indexPath) {
    try {
        const parsed = JSON.parse(await readFile(join(indexPath, "source.json"), "utf8"));
        const sourceDir = typeof parsed === "object" &&
            parsed !== null &&
            "sourceDir" in parsed &&
    ...
    for (;;) {
        try {
            ...
            if (Date.now() - started >= timeoutMs)
                throw new Error(`Timed out waiting for CodeGraph index lock: ${lockPath}`);
...
export class WorkspaceManager {
    ...
    async prepare(identity, options = {}) {
        ...
        const key = createHash("sha256")
            .update(`${identity.repoIdentity}\0${identity.worktreeIdentity}\0${identity.sourcePath}`)
            .digest("hex");
        ...
        try {
            ...
            if (!(await exists(database)))
                await runCodeGraph(this.settings, identity.sourcePath, ["init", "-i"], options);
            const lastSync = this.lastSync.get(identity.sourcePath) || 0;
            const shouldSync = (options.forceSync || this.settings.autoSync) &&
                (options.forceSync ||
    ...
    async #bind(identity, managedIndex) {
        ...
        try {
            ...
            if (info.isSymbolicLink()) {
                ...
                const resolvedIndexStore = await resolvePathSafe(this.settings.indexStore);
                const resolvedSourcePath = await resolvePathSafe(identity.sourcePath);
                const pointsToManagedStore = linkTarget === managedIndex ||
