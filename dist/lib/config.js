...
function normalizeSettings(input) {
    return {
        ...
        codegraphExecutable: typeof input.codegraphExecutable === "string"
...
export async function loadSettings(overrides = {}) {
    const configFile = process.env.PI_CODEGRAPH_CONFIG ||
    ...
    try {
        ...
        if (!hasErrorCode(error, "ENOENT"))
            throw new Error(`Invalid pi-codegraph config at ${configFile}: ${errorMessage(error)}`);
    }
    const environment = {
